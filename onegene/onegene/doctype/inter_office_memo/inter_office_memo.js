...
,    new_supplier_group(frm){
    ...
    validate(frm){
        ...
        calculate_totals(frm);
        if (frm.doc.iom_type === "Approval for New Customer Registration" || frm.doc.iom_type=="Approval for New Supplier Registration") {
         const attachments = frm.doc.attachments || [];

            attachments.forEach(row => {
                if (!row.attach) {
                    frappe.throw("Attachment is mandatory.Kindly attach the documents mentioned in Attachments");
                }
            });
    }
    },
