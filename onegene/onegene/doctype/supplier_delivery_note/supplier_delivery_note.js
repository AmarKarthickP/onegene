frappe.ui.form.on("Supplier Delivery Note", {
    onload(frm) { 
        ...
        frm.fields_dict['item_table'].grid.get_field('purchase_order').get_query = function(doc, cdt, cdn) {
            ...
            frappe.call({
                method: "onegene.onegene.doctype.supplier_delivery_note.supplier_delivery_note.get_query_for_item_table",
                args: {
            ...
            return {
                query: "onegene.onegene.doctype.supplier_delivery_note.supplier_delivery_note.get_query_for_item_table",
                filters: {
                    item_code: item_code,
                    supplier: frm.doc.supplier,
                }
        ...
        frm.fields_dict['item_table'].grid.get_field('item_code').get_query = function(doc, cdt, cdn) {
            return {
                query: "onegene.onegene.doctype.supplier_delivery_note.supplier_delivery_note.get_filter_for_item_code",
                filters: {
                    supplier: frm.doc.supplier,
                }
            };
    ...
    set_query(frm) {
        ...
        frm.set_query('purchase_orders', function() {
            return {
                query: 'onegene.onegene.doctype.supplier_delivery_note.supplier_delivery_note.get_purchase_orders_with_child',
                filters: {
                    supplier: frm.doc.supplier,
                    docstatus: 1,
                    status: ["in", ["To Receive and Bill", "To Receive"]]
    ...
    confirm_supplier_dn(frm) {
        if (frm.doc.confirm_supplier_dn) {
            if (frm.doc.supplier_delivery_note != frm.doc.confirm_supplier_dn) {
                frappe.msgprint(
    ...
    refresh(frm) {
        ...
        frm.fields_dict.html.$wrapper.html(`
            <table class="w-100 mb-3">
                <tr>
                    <td style="padding-left: 10px;"><b>SQ -</b> Scheduled Quantity for Current Month</td>
                    <td style="padding-left: 10px;"><b>PQ -</b> Pending Quantity</td>
                    <td style="padding-left: 10px;"><b>DQ -</b> Dispatch Quantity</td>
                </tr>
                <tr>
    ...
    before_workflow_action: async (frm) => {
        if (frm.doc.workflow_state == "Draft" && frm.selected_workflow_action == "Send for Delivery") {
            await new Promise((resolve, reject) => {
                frappe.call({
                    method: "onegene.onegene.doctype.supplier_delivery_note.supplier_delivery_note.update_purchase_orders",
                    args: {
                        name: frm.doc.name,
                    },
                    callback: function(r) {
        ...
        if (frm.doc.workflow_state == "In Transit" && frm.selected_workflow_action == "Received") {
            await new Promise((resolve, reject) => {
                frappe.call({
                    method: "onegene.onegene.doctype.supplier_delivery_note.supplier_delivery_note.on_submit_action",
                    args: {
                        name: frm.doc.name,
                    },
                    callback: function(r) {
        ...
        if (frm.doc.workflow_state == "Received" && frm.selected_workflow_action == "Cancel") {
            if (frm.doc.confirm_supplier_dn) {
                if (frm.doc.confirm_supplier_dn == frm.doc.supplier_delivery_note) {
                    await new Promise((resolve, reject) => {
                        frappe.call({
                            method: "onegene.onegene.doctype.supplier_delivery_note.supplier_delivery_note.on_cancel_action",
                            args: {
                                name: frm.doc.name,
                            },
                            callback: function(r) {
    ...
    update_items(frm) {
        if (frm.doc.purchase_order){
            ...
            frappe.call({
                method: "onegene.onegene.doctype.supplier_delivery_note.supplier_delivery_note.get_all_item_schedule_data",
                args: {
                    purchase_order: frm.doc.purchase_order,
    ...
    //         frappe.call({
    //             method: "onegene.onegene.doctype.supplier_delivery_note.supplier_delivery_note.get_all_item_schedule_data_purchase_orders",
    //             args: {
...
frappe.ui.form.on('Supplier-DN Item', {
    ...
    item_code: function(frm, cdt, cdn) {
        ...
        frappe.call({
            method: "onegene.onegene.doctype.supplier_delivery_note.supplier_delivery_note.validate_item_code",
            args: {
    ...
    purchase_order: function(frm, cdt, cdn) {
        ...
        if (purchase_order && current_row.item_code) {
            frappe.call({
        method: "onegene.onegene.doctype.supplier_delivery_note.supplier_delivery_note.validate_purchase_order",
        args: {
        ...
        callback: function(r) {
            ...
            frappe.call({
                method: "onegene.onegene.doctype.supplier_delivery_note.supplier_delivery_note.get_item_schedule_data",
                args: {
                    purchase_order: purchase_order,
                    item_code: current_row.item_code,
                    date_time: frm.doc.datetime,
