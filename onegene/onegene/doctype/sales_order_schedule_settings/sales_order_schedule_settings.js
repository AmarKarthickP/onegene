...
frappe.ui.form.on('Sales Order Schedule Settings', {
	refresh(frm) {
        
        frm.fields_dict.html_1.$wrapper.html(`
            <p style="font-size: 15px;">A) Download the template of sales order schedule</p>
        `);
        ...
        frappe.realtime.on('sales_order_upload_progress', function(data) {
            ...
            frappe.upload_dialog.set_title(__(data.stage || 'Creating Sales Order Schedule'));
        ...
        frappe.realtime.on("so_upload_summary", (data) => {
            const response_data = `
            <div style="
                max-width: 420px;
                margin: 20px auto;
    ...
    attach(frm) {
        if (frm.doc.attach) {
            frappe.call({
                method: 'onegene.onegene.doctype.sales_order_schedule_settings.updated_so_schedule_settings.enqueue_upload_validate',
