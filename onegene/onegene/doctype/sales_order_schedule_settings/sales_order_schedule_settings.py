...
from onegene.onegene.doctype.open_order.open_order import update_child_qty_rate
...
def enqueue_upload(file):
	...
	pps = [pp for pp in pps if pp and pp[0] != "Customer Code"]
...
def _process_upload(file, records):
	...
	def publish_progress(current_step, total_steps, stage):
		frappe.publish_realtime(
			event='sales_order_upload_progress',
			message={
		...
		try:
			...
			if sales.customer_order_type == "Open":
				...
				if not frappe.db.exists("Sales Order Schedule", {
					...
					doc.tentative_plan_1 = tp_1
					doc.tentative_plan_2 = tp_2
					doc.disable_update_items = 1
					doc.save(ignore_permissions=True)

					docs_to_submit.append(doc.name)
					grouped_docs[so_number].append(doc.name)
					...
					if not frappe.db.exists("Sales Order Schedule", {
						...
						doc.append("revision", {
							...
							"remarks": "Revised from Sales Order Schedule Settings"
						})

						pending_qty = s_qty - doc.delivered_qty
			...
			frappe.log_error(f"Row failed: {e}", "Sales Order Upload Error")

		current_step += 1
		publish_progress(current_step, total_steps, "Creating Schedule")

	# === STEP 2: Submit Sales Order Schedules ===
	for name in docs_to_submit:
		try:
			...
			if doc.docstatus == 0:
				...
				if doc.order_type == "Open" and frappe.db.exists("Sales Order", {
					'name': doc.sales_order_number, 'docstatus': 1
				}):
					...
					if matching_row:
						open_order.disable_update_items = 0 if doc.disable_update_items == 0 else 1
						matching_row.qty = item_qty
						matching_row.amount = item_qty * float(doc.order_rate)
						open_order.save(ignore_permissions=True)

		except Exception as e:
			frappe.log_error(f"Submit failed for {name}: {e}", "Sales Order Upload Error")
		finally:
	...
	frappe.log_error("Disctince SO Number", distinct_so_numbers)
	# === STEP 3: Update Open Orders ===
	distinct_so_numbers = list(set(distinct_so_numbers))
	for so_number in distinct_so_numbers:
		try:
			order_open = frappe.get_doc("Open Order", {
				"sales_order_number": so_number
			})
	...
	"""
	frappe.db.set_single_value("Sales Order Schedule Settings", "response_data", response_data)
	frappe.clear_cache()
	frappe.publish_realtime("so_upload_summary", {
		"new": new_count,
		"update": update_count,
		"skip": skipped_count
	})
...
def precheck_records(file,records,job_id):
	...
	for idx, pp in enumerate(records, start=1):
		try:
			...
			existing = frappe.db.exists("Sales Order Schedule", {
				'sales_order_number': so_number,
				'schedule_month': schedule_month,
				'item_code': item,
				'docstatus': 1
			...
			frappe.log_error(f"Row {idx}: {str(e)}", "Sales Order Upload Error")
		...
		try:
			...
			if doc.docstatus == 1:
				if doc.order_type == "Open" and frappe.db.exists("Sales Order", {'name': doc.sales_order_number, 'docstatus': 1}):
					...
					schedules = frappe.get_all("Sales Order Schedule",{"sales_order_number": doc.sales_order_number,"item_code": doc.item_code,"docstatus": 1},["qty"])
					item_qty = sum(s.qty for s in schedules)

					matching_row = next((row for row in open_order.open_order_table if row.item_code == doc.item_code), None)
					if not matching_row:
						raise Exception(f"No matching row found in Open Order for item {doc.item_code}")
	...
	if error_logs:
		...
		frappe.log_error(error_message, "Sales Order Upload Error")
	...
	for idx, sales_order_number in enumerate(distinct_so_numbers, start=1):
		try:
			...
			error_logs.append(f"PO {sales_order_number}: {str(e)}")

	# if error_logs:
	# 	error_message = "Errors occurred while updating Open Orders:\n" + "\n".join(error_logs)
	# 	frappe.log_error(error_message, "Sales Order Upload Error")
	if error_logs:
		# error_message = "\n".join(error_logs)
		frappe.throw('')
		...
		frappe.enqueue(
			method="onegene.onegene.doctype.sales_order_schedule_settings.sales_order_schedule_settings._process_upload",
			queue="long",
			timeout=1800,
			is_async=True,
			file=file,
