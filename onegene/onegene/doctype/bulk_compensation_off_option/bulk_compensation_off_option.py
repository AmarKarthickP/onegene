	...
	def mark_compoff(self):
		...
		for i in employees_list:
			if self.compensation_marked == 0:
				if not frappe.db.exists("Attendance",{"docstatus":1,'status':'Present','attendance_date':self.compensation_off_date,'employee':i.employee}):
					...
					if leave_period:
						...
						if leave_allocation :
							for allocation in leave_allocation:
								new_leave = allocation['new_leaves_allocated'] + 0
