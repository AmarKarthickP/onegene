...
def get_columns(filters):
	cols=[
			...
			{"label": _("Item Name"), "fieldtype": "Data", "fieldname": "item_name", "width": 450},
			...
			{"label": _("Customer"), "fieldtype": "Data", "fieldname": "customer", "width": 200},
...
def get_data(filters):
	...
	query = """
		SELECT customer_name, item_code, item_name, item_group, SUM(qty) as qty, SUM(delivered_qty) as delivered_qty
...
def get_customer_wise_summary(data):
	...
	for row in data:
		...
		<tr>
			<td>{row['customer']}</td>
			<td>{row['today_customer_plan' or 0]}</td>
			<td>{row['commitment_plan' or 0]}</td>
			<td></td>
			<td></td>
			<td></td>
			<td></td>
