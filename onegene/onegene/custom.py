...
def validate_sales_invoice(doc, method):
    ...
    if not doc.transporter:
        if frappe.db.exists("Logistics Request",{"order_no":doc.name}):
            ffw=frappe.db.get_value("Logistics Request",{"order_no":doc.name},['recommended_ffw'])
            if ffw:
                doc.transporter=ffw
    for row in doc.items:
        if row.qty > 0:
...
def create_user_for_supplier(doc, method):
    ...
    if doc.email_id:
        if not frappe.db.exists("User", {"email": doc.email_id}):
            ...
            user.role_profile_name =  "OUTSOURCING"
...
@frappe.whitelist()
def update_tax_category(doc,method):
    if doc.customer_group!='Export' and not doc.tax_category:
        if doc.customer_primary_address:
            ...
            if frappe.db.exists("Dynamic Link",{'parenttype':'Address',"link_doctype":'Customer',"link_name":doc.name}):
                address=frappe.get_doc("Dynamic Link",{'parenttype':'Address',"link_doctype":'Customer',"link_name":doc.name})
                if address.country=='India':
                    if address.state=='Tamil Nadu' :
                        doc.tax_category='In-State'
                    else:
...
# def test_check():
#     data = frappe.db.sql("""
#         SELECT c.item_code, c.allocated_qty, c.parent, p.workflow_state, p.supplier
#         FROM `tabAdvance Shipping Note` p
#         INNER JOIN `tabSupplier-DN Schedule Trace` c
#             ON c.parent = p.name
#         WHERE c.item_code = 'HC864-K8LAA-01'
#             AND DATE(p.datetime) BETWEEN '2025-11-01' AND '2025-11-30'
#         """, as_dict=1)

#     sum = 0
#     for row in data:
#         print(row)
#         sum += row.allocated_qty
#     print(sum)

#     data = frappe.db.sql("""
#         SELECT c.item_code, c.dis_qty, c.parent, p.workflow_state, p.supplier
#         FROM `tabAdvance Shipping Note` p
#         INNER JOIN `tabSupplier-DN Item` c
#             ON c.parent = p.name
#         WHERE c.item_code = 'HC864-K8LAA-01'
#             AND DATE(p.datetime) BETWEEN '2025-11-01' AND '2025-11-30'
#         """, as_dict=1)
...
def test_check():
	duplicates = frappe.db.sql("""
		SELECT parent AS purchase_order, item_code, COUNT(item_code) AS qty
...
def remove_duplicate_po_items():
	...
	for row in duplicates:
		...
		# get all duplicated rows ordered by creation time
		rows = frappe.get_all(
			"Purchase Order Item",
			filters={"parent": po, "item_code": item},
			fields=["name"],
			order_by="creation asc"
		)
		...
		for r in rows[1:]:
			frappe.db.sql("delete from `tabPurchase Order Item` where name = %s", (r.name))

		print(f"Duplicates removed from PO {po} for item {item}")

	return "Duplicate cleanup complete"
